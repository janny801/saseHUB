...
async function loadAlumni() {
    ...
    dataSection.style.display = "block"; // Fix: Ensure section is visible
    dataSection.innerHTML = "<p>Loading Alumni Database...</p>";
    ...
    try {
        ...
        dataSection.innerHTML = "<p style='color:red;'>Error loading alumni data.</p>";
    }
}
...
async function loadProfessors() {
    const dataSection = document.getElementById("dataSection");
    dataSection.style.display = "block"; // Fix: Ensure section is visible
    dataSection.innerHTML = "<p>Loading Professor Database...</p>";

    try {
        const res = await fetch("/professors/all");
        const data = await res.json();
        renderProfessorsTable(data);
    } catch (err) {
        console.error("Error loading professors:", err);
        dataSection.innerHTML = "<p style='color:red;'>Error loading professor data.</p>";
    }
}
...
function renderMajorDropdown(list) {
    ...
    list.forEach(m => {
        ...
        item.innerHTML = `<span>${m.major_name.trim()}</span>`;
...
function updateMajorTags() {
    ...
    document.getElementById("selected_major_ids").value = selectedMajors.map(m => m.id).join(",");
}
...
async function saveProfessor() {
    const formData = new FormData();
    formData.append("professor_name", document.getElementById("prof_name").value);
    formData.append("email", document.getElementById("prof_email").value);
    formData.append("university", document.getElementById("prof_university").value);
    formData.append("description", document.getElementById("prof_desc").value);
    formData.append("major_ids", selectedMajors.map(m => m.id).join(","));

    if (document.getElementById("prof_pic").files[0]) {
        formData.append("profile_pic", document.getElementById("prof_pic").files[0]);
    }
...
async function loadMajorsEdit(initialMajorNames) {
    ...
    if (initialMajorNames) {
        const names = initialMajorNames.split(",").map(s => s.trim()).filter(s => s.length > 0);
        names.forEach(n => {
            const match = editMajorsCache.find(m => m.major_name.trim().toLowerCase() === n.toLowerCase());
...
function filterMajorsEdit() {
    ...
    const filtered = editMajorsCache.filter(m => m.major_name.toLowerCase().includes(query));
...
function renderMajorDropdownEdit(list) {
    ...
    list.forEach(m => {
        const item = document.createElement("div");
        item.className = "dropdown-item";
        if (editSelectedMajors.find(x => x.id === m.id)) item.classList.add("selected");
        item.innerHTML = `<span>${m.major_name.trim()}</span>`;
        item.onclick = function() { toggleEditMajor(m); };
        container.appendChild(item);
    });
}
...
function toggleEditMajor(major) {
    ...
    if (exists) editSelectedMajors = editSelectedMajors.filter(m => m.id !== major.id);
    else editSelectedMajors.push(major);
    updateEditMajorTags();
    renderMajorDropdownEdit(editMajorsCache);
}
...
function updateEditMajorTags() {
    ...
    document.getElementById("edit_selected_major_ids").value = editSelectedMajors.map(m => m.id).join(",");
}
...
async function saveProfessorUpdate() {
    ...
    formData.append("professor_name", document.getElementById("edit_prof_name").value);
    formData.append("email", document.getElementById("edit_prof_email").value);
    formData.append("university", document.getElementById("edit_prof_university").value);
    formData.append("description", document.getElementById("edit_prof_desc").value);
    formData.append("major_ids", editSelectedMajors.map(m => m.id).join(","));

    if (document.getElementById("edit_prof_pic").files[0]) {
        formData.append("profile_pic", document.getElementById("edit_prof_pic").files[0]);
    }

    const res = await fetch("/professors/update", { method: "POST", body: formData });
    const data = await res.json();
    alert(data.message);
...
async function confirmDeleteProfessor() {
    const res = await fetch("/professors/delete/" + deleteProfID, { method: "DELETE" });
    const data = await res.json();
    alert(data.message);
...
function toggleAlumniMajorDropdown() {
    ...
    // Smooth scroll to the list so the user sees it opened
    if (!isOpen) {
...
function filterAlumniMajors() {
    ...
    const filtered = alumniMajorsCache.filter(m => m.major_name.toLowerCase().includes(query));
...
function renderAlumniMajorDropdown(list) {
    ...
    list.forEach(m => {
        const item = document.createElement("div");
        item.className = "dropdown-item";
        if (alumniSelectedMajors.find(x => x.id === m.id)) item.classList.add("selected");
        item.innerHTML = `<span>${m.major_name}</span>`;
...
function toggleAlumniMajor(major) {
    ...
    if (exists) alumniSelectedMajors = alumniSelectedMajors.filter(m => m.id !== major.id);
    else alumniSelectedMajors.push(major);
...
function updateAlumniMajorTags() {
    ...
    document.getElementById("alumni_selected_major_ids").value = alumniSelectedMajors.map(m => m.id).join(",");
}
...
function filterAlumniGoals() {
    ...
    const filtered = alumniGoalsCache.filter(g => g.goal_name.toLowerCase().includes(q));
...
function renderAlumniGoalDropdown(list) {
    ...
    list.forEach(g => {
        const item = document.createElement("div");
        item.className = "dropdown-item";
        if (alumniSelectedGoals.find(x => x.id === g.id)) item.classList.add("selected");
...
function toggleAlumniGoal(goal) {
    ...
    if (exists) alumniSelectedGoals = alumniSelectedGoals.filter(g => g.id !== goal.id);
    else alumniSelectedGoals.push(goal);
...
function updateAlumniGoalTags() {
    ...
    document.getElementById("alumni_selected_goal_ids").value = alumniSelectedGoals.map(g => g.id).join(",");
}
...
async function loadAlumniGoalsEdit(initialGoals) {
    ...
    if (initialGoals) {
        ...
        names.forEach(n => {
            const match = editAlumniGoalsCache.find(g => g.goal_name.trim().toLowerCase() === n.toLowerCase());
...
function filterAlumniGoalsEdit() {
    ...
    const filtered = editAlumniGoalsCache.filter(g => g.goal_name.toLowerCase().includes(q));
...
function renderAlumniGoalDropdownEdit(list) {
    ...
    list.forEach(g => {
        const item = document.createElement("div");
        item.className = "dropdown-item";
        if (editAlumniSelectedGoals.find(x => x.id === g.id)) item.classList.add("selected");
...
function toggleEditAlumniGoal(goal) {
    ...
    if (exists) editAlumniSelectedGoals = editAlumniSelectedGoals.filter(g => g.id !== goal.id);
    else editAlumniSelectedGoals.push(goal);
    updateAlumniGoalTagsEdit();
    renderAlumniGoalDropdownEdit(editAlumniGoalsCache);
}
...
function updateAlumniGoalTagsEdit() {
    ...
    box.innerHTML = "";
    document.getElementById("edit_alumni_goal_search").value = editAlumniSelectedGoals.map(g => g.goal_name).join(", ");
    ...
    document.getElementById("edit_alumni_selected_goal_ids").value = editAlumniSelectedGoals.map(g => g.id).join(",");
}
...
async function loadAlumniMajorsEdit(initialMajors) {
    ...
    if (initialMajors) {
        ...
        names.forEach(n => {
            const match = editAlumniMajorsCache.find(m => m.major_name.trim().toLowerCase() === n.toLowerCase());
            if (match) editAlumniSelectedMajors.push(match);
        });
    }
    document.getElementById("edit_alumni_major_search").value = editAlumniSelectedMajors.map(m => m.major_name).join(", ");
...
function filterAlumniMajorsEdit() {
    ...
    const filtered = editAlumniMajorsCache.filter(m => m.major_name.toLowerCase().includes(query));
...
function renderAlumniMajorDropdownEdit(list) {
    ...
    list.forEach(m => {
        const item = document.createElement("div");
        item.className = "dropdown-item";
        if (editAlumniSelectedMajors.find(x => x.id === m.id)) item.classList.add("selected");
        item.innerHTML = `<span>${m.major_name}</span>`;
...
function toggleEditAlumniMajor(major) {
    ...
    if (exists) editAlumniSelectedMajors = editAlumniSelectedMajors.filter(m => m.id !== major.id);
    else editAlumniSelectedMajors.push(major);
    updateAlumniMajorTagsEdit();
    renderAlumniMajorDropdownEdit(editAlumniMajorsCache);
}
...
function updateAlumniMajorTagsEdit() {
    ...
    document.getElementById("edit_alumni_selected_major_ids").value = editAlumniSelectedMajors.map(m => m.id).join(",");
}
